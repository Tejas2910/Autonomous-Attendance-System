...
void delay_ms(int del){
    ...
    //SysCtlDelay(n) , it waits for n loops. each loop is of 3 clock cycles.
    // waiting for "del" clock cycles is equivalent to waiting for "del/3" cycles.
    uint32_t del1 = del*(SysCtlClockGet() / (1000*3));
    SysCtlDelay(del1);
}
...
void drawNumber(int n, int16_t x, int16_t y, uint16_t colour, uint16_t bg, uint8_t size){
    cursor_x = x;
    cursor_y = y;
    textsize = size;
    textcolour = colour;
    ...
    while(len){
        ascii = n/len + 48;
        drawChar(cursor_x, cursor_y, ascii, textcolour, textbgcolour, textsize);
        n = n%len;
        len = len/10;
        cursor_x += textsize*6;
        if (wrap && (cursor_x > (width - textsize*6))) {
